...
const ConsumptionGraphEnergy = () => {
  ...
  const fetchStackOptions = async (userName) => {
    try {
      ...
      const filteredStacks = response.data.stackNames.filter(stack => stack.stationType === 'energy');
  ...
  const processData = () => {
    ...
    const datasets = stackNames.map((name, index) => {
      ...
      const stackData = graphData.map(entry => entry.stacks.find(s => s.stackName === name)?.energyMonthlyConsumption || 0);
  ...
  const chartOptions = {
    ...
    plugins: {
      ...
      title: {
        display: true,
        text: 'Monthly Energy Consumption Over Time',
      },
    },
    scales: {
  ...
  return (
    <div className="energy-flow-container">
      <h3 className="text-center mb-4">Monthly Energy Consumption</h3>
...
export default ConsumptionGraphEnergy;

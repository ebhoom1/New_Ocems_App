...
const ConsumptionPredictionGraph = () => {
    ...
    const processData = () => {
        if (!Array.isArray(graphData) || graphData.length === 0) {
            return { labels: [], inflowValues: [], finalFlowValues: [] };
        }
        ...
        const inflowValues = graphData.map((entry) => {
            const data = dataType === 'consumption'
                ? entry.totalConsumptionData?.[0]
                : entry.predictionData?.[0];
            return data ? data.inflow || data.predictedInflow || 0 : 0;
        });

        const finalFlowValues = graphData.map((entry) => {
            const data = dataType === 'consumption'
                ? entry.totalConsumptionData?.[0]
                : entry.predictionData?.[0];
            return data ? data.finalflow || data.predictedFinalflow || 0 : 0;
        });

        return { labels, inflowValues, finalFlowValues };
    };

    const { labels, inflowValues, finalFlowValues } = processData();
...
export default ConsumptionPredictionGraph;
